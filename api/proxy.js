...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const response = await axios({
            ...
            headers: {
                'x-api-key': ADMIN_API_KEY,  // YAHAN PE API KEY LAG RAHI HAI
        ...
        if (error.response) {
            ...
            console.error('Main app error:', error.response.status, error.response.data);
            return res.status(error.response.status).json(error.response.data);
        } else if (error.request) {
            // No response from main app
            return res.status(503).json({ 
                error: 'Main app not responding',
                details: 'Check if Railway app is running'
            });
        } else {
            // Other error
